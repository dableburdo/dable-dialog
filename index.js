/**
 * @file dable-dialog/index.js
 * @brief dialog component for dable
 *        modal dialog, but hide when clicking outside the dialog
 * @author simpart
 */
const Modal   = require('dable-modalfil');
const Frame   = require('dable-ttlframe');
const Text    = require('dable-text');
const Button  = require('dable-button');
const Click   = require('dable-event-click');
const ClkFcs  = require('dable-event-clkfocus');
const vsClick = require('dable-event-visiclick');
const HrzPos  = require('dable-effect-hrzpos'); 
const VrtPos  = require('dable-effect-vrtpos');
const SyncHei = require('dable-effect-synchei');
const SyncWid = require('dable-effect-syncwid');
const Fade    = require('dable-effect-fade');
const comutl  = dable.util.common;

module.exports = class extends dable.class.Component {
    /**
    ...
    constructor (p1) {
        try {
            ...
            this.confmng().add("openEvent", { type: "event", list: true });
	    this.confmng().add("closeEvent", { type: "event", list: true });
    ...
    initDomConts () {
	    ...
	    this.frame(new Frame({
	        effect: [ new Fade({ tag: "Dialog" }), new Fade({ value: false, tag: "Dialog", eid:1 }) ]
            }));
	    this.btnWrap(new dable.class.Component());
		...
		effect: [ new Fade({ tag:"Dialog" }), new Fade({ value: false, tag:"Dialog", eid: 1 }) ]
            ...
            this.closeComp(
                new Text({
                    style : new dable.class.ConfArg({ "font-family" : "auto" }, { locked: true }),
	    ...
	    this.frame().header().bind(true);
            ...
            let conts = new dable.class.Component({
     ...
     * @return (dable-text) title text component
     * @type parameter
     */
    title (prm, cnf) {
        try {
     ...
     * @return (dable-button) dialog button component
     * @type parameter
     */
    button (prm, opt) {
        try {
    ...
    buttonEvent (fnc, prm) {
        try {
	    if ((undefined !== fnc) && (true === this.isExists())) {
                let btn = this.button();
		for (let bidx in btn) {
                    btn[bidx].clickEvent(fnc, prm);
		}
	    }
     ...
     * dialog frame
     * 
     * @param (dable-frame) dialog frame component
     * @return (dable-frame) dialog frame component
     * @type parameter
     */
    frame (prm) {
        try {
		    ...
		    style     : { "position" : "relative", "display" : "none" },
                    header    : new dable.class.PullConf({ height: '0.4rem' }),
                    mainColor : [230, 230, 230], baseColor : 'white',
		    event     : new ClkFcs({
                                    listener: new dable.class.ConfArg(fcs,this),
     ...
     * 
     * @param (dable-modalfil) modal filter component
     * @return (dable-modalfil) modal filter component
     * @type private
     */
    modalfil (prm) {
       try {
    ...
    fade (prm) {
        try {
            let eff = this.frame().effect({ modname:"Fade", tag:"Dialog" });
	    for (let eidx in eff) {
                eff[eidx].speed(prm);
	    }
	    eff = this.modalfil().effect({ modname:"Fade", tag:"Dialog" });
	    for (let eidx_2 in eff) {
                eff[eidx_2].speed(prm);
	    }
	} catch (e) {
            console.error(e.stack);
            throw e;
    ...
    openEvent (fnc, prm) {
        try {
            if (undefined === fnc) {
                /* getter */
                return this.confmng("openEvent");
            }
            /* setter */
            this.confmng("openEvent", [fnc,prm]);
	} catch (e) {
            console.error(e.stack);
            throw e;
	}
    ...
    closeEvent (fnc, prm) {
        try {
            if (undefined === fnc) {
                /* getter */
                return this.confmng("closeEvent");
            }
            /* setter */
            this.confmng("closeEvent", [fnc,prm]);
	} catch (e) {
            console.error(e.stack);
            throw e;
	}
